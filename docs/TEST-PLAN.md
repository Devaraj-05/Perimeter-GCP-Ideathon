...
| Element | What it does |
|---|---|
| **Fire the whole corpus** | Runs all 19 payloads (14 authored P01-P14, 5 published T01-T05); shows N/19 blocked |
...
5. Attack it → Fire the whole corpus → **19/19 blocked**. *(TC-D1)*
...
```bash
npm test            # 332 unit tests (airlock, broker, SSRF, detection, INV guards)
npm run test:rules  # 56 Firestore rules tests including cross-user and tamper cases
npm run replay      # two corpus tables: authored 14/14, third-party 5/5
```
