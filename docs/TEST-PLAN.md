...
| Element | What it does |
|---|---|
| **Fire the whole corpus** | Runs all 24 payloads (19 authored P01-P19, 5 published T01-T05); shows N/24 blocked |
...
5. Attack it → Fire the whole corpus → **24/24 blocked**. *(TC-D1)*
...
```bash
npm test            # 415 unit tests (airlock, broker, SSRF, detection, INV guards)
npm run test:rules  # 66 Firestore rules tests including cross-user and tamper cases
npm run replay      # two corpus tables: authored 19/19, third-party 5/5
```
